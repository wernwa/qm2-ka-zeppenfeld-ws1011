...
Was hier im Integral unterschlagen (oder unsichtbar) ist, dass die Kets und bras im WW-Bild von der Zeit Abhängen. Es kommen noch von beiden Seiten Exponetial Funktionen heraus, falls man das Schrödinger Bild wieder betrachtet. Zweck dieser Übung ist, die Matrixelemente \(V_I\) aus dem Integral auszuklammern:

\begin{align}
c_n^{(1)}(t) &= \delta_{ni} -  \frac{i}{\hbar}\int_{t_0}^{t'} \langle n|e^{\frac{i}{\hbar}E_nt'} V_I e^{-\frac{i}{\hbar}E_it'} |i\rangle dt' \\
&= \delta_{ni} -  \frac{i}{\hbar}\langle n|V_I  |i\rangle \int_{t_0}^{t'} dt' e^{\frac{i}{\hbar}(E_n-E_i)t'} \\
&= \delta_{ni} -  \frac{i}{\hbar}\langle n|V_I  |i\rangle \int_{t_0}^{t'} dt e^{i\omega_{ni}t'}
\end{align}

Mit \(\omega_{ni} =\frac{i}{\hbar}(E_n-E_i) \). 
